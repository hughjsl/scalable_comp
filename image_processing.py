"""
    simple image processing - compresses colours into binary colour space, removes dot noise,
    attempts to remove underlines

    class was originally intended to also split the captchas per character to simplify training, but
    gave up on this, so there a lot of unused code here.
"""
...
class PreprocConfig:
    # colour compression
    ADAPT_BLOCK: int = 31
    ADAPT_C: int = 7               # tuning to account for anti aliasing
    ERODE_AFTER_BIN: bool = True   # 1px shave
    ...
    DOT_MAX_H: int = 8             #
    DOT_MAX_AREA: int = 49         #
    DOT_MARGIN: int = 2            # ring of white space required around "dot"
    DOT_ALLOW_RING_FG: int = 2     # tolerance, pixels allowed in ring

    # object size filter after processing - unused
    MIN_W: int = 5
    MIN_H: int = 10

    # attempts to ignore underline when creating boxes
...
def load_rgb(img: Union[str, Path, np.ndarray]) -> np.ndarray:
    ...
    if arr.ndim != 3 or arr.shape[2] != 3:
        raise ValueError("expected size (H, W, 3).")
...
def remove_isolated_dots(bw: np.ndarray, cfg: PreprocConfig) -> Tuple[np.ndarray, int]:
    """
    remove small and unconnected dots
    """
...
def find_components(clean_bw: np.ndarray, cfg: PreprocConfig) -> List[Tuple[int, int, int, int]]:
    """
    bounding box generation - does not work, not used. training and classifier both use whole images
    """
